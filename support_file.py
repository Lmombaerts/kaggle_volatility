...
def trade_preprocessor(trades_stock, stock_id):
    ...
    def get_stats_window(seconds_in_bucket, add_suffix = False):
        ...
        if trades_stock_sub.empty == False:
            ...
            df_feature = pd.concat([df_feature,roll_measure['roll_measure'],roll_impact['roll_impact'],amihud['amihud'],traded_volume['traded_volume'],avg_trade_size['avg_trade_size']],axis=1)
        else:
...
def trade_book_preprocessor(df_book, df_trades, stock_id):
    ...
    # Time Length (for next computation) - this should be done after grouping
    df_book['time_length'] = df_book.groupby('time_id')['seconds_in_bucket'].diff().shift(periods=-1)
    final_observations = df_book.index[np.isnan(df_book['time_length'])].tolist()
    df_book.loc[final_observations, 'time_length'] = 600 - df_book.loc[final_observations, 'seconds_in_bucket']
    
    # Adding trade columns
    df_trades['prev_trade_second'] = df_trades['seconds_in_bucket'].shift()
    
    # Group by time window
    unique_times = df_trades['time_id'].unique()

    vpins = []
    for time_id in unique_times:
        trades_subgroup = df_trades.groupby(['time_id']).get_group(time_id)
        book_subgroup = df_book.groupby(['time_id']).get_group(time_id)
        trades_subgroup['buyer_volume'] = trades_subgroup.apply(lambda row: signed_volume(book_subgroup, row['prev_trade_second'], row['seconds_in_bucket'], row['price'], row['size']), axis=1)
        trades_subgroup['seller_volume'] = trades_subgroup['size'] - trades_subgroup['buyer_volume']
        trades_subgroup['vpin'] = trades_subgroup.apply(lambda row: np.abs(row['buyer_volume'] - row['seller_volume'])/row['size'], axis = 1)
        vpins.append(np.sum(trades_subgroup['vpin'] * trades_subgroup['size']) / np.sum(trades_subgroup['size']))
...
def signed_volume(df_book, start_time, end_time, price, volume, output='buy'):
    ...
    sub_book = df_book[(start_time <= df_book['seconds_in_bucket']) & (df_book['seconds_in_bucket'] < end_time)]
    w_ask = np.sum(sub_book['ask_price1'] * sub_book['time_length']) / (end_time - start_time)
    w_bid = np.sum(sub_book['bid_price1'] * sub_book['time_length']) / (end_time - start_time)
